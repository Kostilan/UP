        ...
        <div class="genre-left">
            <div class="genre">
                <ul class="genre-list">
                    ...
                    <li class="list-group-item"><a class="link-dark link-underline-light" href="/account">Обо мне</a></li>
                    <li class="list-group-item"><a class="link-dark link-underline-light" href="/account/accountBookMarks">Избранное</a></li>
                    <li class="list-group-item"><a class="link-dark link-underline-light" href="{{ route('accountComments') }}">Мои комментарии</a></li>
                    <li class="list-group-item"><a class="link-dark link-underline-light" href="{{ route('accountRead') }}">Я читаю</a></li>
                </ul>
            </div>
        </div>
                    ...
                    <div class="d-flex book-list">
                        @foreach ($chunkedBooks as $book)
                            <div class="book-account_marks book">
                                ...
                                <p>Автор: <a href="{{ route('authorsBooks', ['id' => $book->author->id]) }}">{{ $book->author->surname_author }} {{ $book->author->name_author }}</a></p>
                                ...
                                @php
                                    $note = $notes->where('book_id', $book->id)->first();
                                    // dd($note); // Временный вывод для отладки
                                    $currentPage = $note ? $note->page : 0;
                                    $totalPages = $note->all_pages;
                                    $progress = $totalPages > 0 ? ($currentPage / $totalPages) * 100 : 0;
                                @endphp

                                <p>Прогресс чтения: {{ round($progress) }}%</p>
                                <div class="progress">
